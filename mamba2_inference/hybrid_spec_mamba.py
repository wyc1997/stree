...
from mamba2.spec_mamba_layer import Mamba2
...
class MHADecoderLayer(nn.Module):
    ...
    def allocate_value_cache(self, batch_size, max_seqlen, dtype=None, **kwargs):
        return self.mha.allocate_value_cache(batch_size, max_seqlen, dtype=dtype, **kwargs)
    
    def forward(self, hidden_states: Tensor, position_ids=None, mask=None, inference_params=None, *args, **kwargs):
        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = self.mha(hidden_states, position_ids, mask, inference_params)
...
class SpecMambaDecoderLayer(nn.Module):
    def __init__(
        self,
        config: MambaConfig,
        layer_idx: int,
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        super(SpecMambaDecoderLayer, self).__init__()
        self.layer_idx = layer_idx
        self.mamba = Mamba2(
    ...
    def allocate_value_cache(self, batch_size, max_seqlen, dtype=None, **kwargs):
        return self.mamba.allocate_value_cache(batch_size, max_seqlen, dtype=dtype, **kwargs)
    
    def forward(self, hidden_states: Tensor, mask=None, inference_params=None, *args, **kwargs):
        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = self.mamba(hidden_states, mask=mask, inference_params=inference_params)
        hidden_states = residual + hidden_states
