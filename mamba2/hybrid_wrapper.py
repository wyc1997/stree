...
class MambaTransformerHybridModelWrapper(nn.Module):
    ...
    def __init__(self, checkpoint_path, transformer_model, mamba_config, attn_layers, dtype, init_with_kqvo, load_from_hub=False, **kwargs):
        ...
        if checkpoint_path is not None:
            if load_from_hub:
                ...
                state_dict = load_state_dict_hf(checkpoint_path, device=torch.device("cpu"), dtype=dtype)
                ...
                if os.path.exists(f"{checkpoint_path}/pytorch_model.bin"):
                    ...
                    state_dict = torch.load(f"{checkpoint_path}/pytorch_model.bin", map_location=torch.device("cpu"))
                    with deepspeed.zero.GatheredParameters(list(self.model.parameters()), modifier_rank=0):
                        self.model.load_state_dict(state_dict)
                else:
    ...
    def from_pretrained_local(pretrained_model_name, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2"):
        ...
        transformer_config = AutoConfig.from_pretrained(config_data["_name_or_path"])
        transformer_model = AutoModelForCausalLM.from_config(transformer_config, torch_dtype=torch_dtype, attn_implementation=attn_implementation)
        with open(f'{pretrained_model_name}/{MAMBA_CONFIG_NAME}', 'r') as json_file:
            config_dict = json.load(json_file)
        mamba_config = MambaConfig(**config_dict)
        with deepspeed.zero.Init():
            m = MambaTransformerHybridModelWrapper(pretrained_model_name, transformer_model, mamba_config, mamba_config.attn_layers, torch_dtype, init_with_kqvo=False) 
        return m
    ...
    def from_pretrained_hub(pretrained_model_name, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2"):
        ...
        mamba_config = MambaConfig(**config_dict)
        with deepspeed.zero.Init():
            m = MambaTransformerHybridModelWrapper(pretrained_model_name, transformer_model, mamba_config, mamba_config.attn_layers, torch_dtype, init_with_kqvo=False, load_from_hub=True) 
        return m
