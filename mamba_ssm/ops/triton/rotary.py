...
def rotary_kernel(
    ...
    if not HAS_POSITION_IDS:
        ...
        POSITION_IDS += pid_batch * seqlen + rm # position ids
        rm_cs = tl.load(POSITION_IDS, rm<seqlen, other=0.0).to(tl.int64)
...
def apply_rotary(
    ...
    Arguments:
        ...
        position_ids: (batch, seqlen) tensor to indicate the position for ratary embedding to apply, overwrites seqlen_offsets argument
                    (i.e. when position ids is supplied, seqlen_offsets will be ignored)
    ...
    assert not (is_varlen and has_position_ids), "varlen and position ids can't both be not None" #TODO: 
    if not is_varlen:
        ...
        seqlen = max_seqlen
    if has_position_ids:
        assert position_ids.shape == (batch, seqlen)
    ...
    with torch.cuda.device(x.device.index):
        # torch.library.wrap_triton(rotary_kernel)[grid]( # torch 2.3.0 doesn't have wrap_triton API
        rotary_kernel[grid](
            output,  # data ptrs
            x,
            cos,
